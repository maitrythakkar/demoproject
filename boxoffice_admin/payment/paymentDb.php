...
//<!-- header-starts -->
		<div class="sticky-header header-section ">
	      ...
	      <input  type="submit" name="submit"   value="Search">
						-->
						<input   placeholder="Search by payment-id"  name="name" type="text"class="cls" id="serchform" />
 ...
 //-query  the database table 
	  $sql="SELECT  pk_Payment_id,fk_email_id,fk_book_id,Ticket_charge,Other_charge, Total FROM payment_tbl WHERE pk_Payment_id LIKE '%" . $name .  "%'"; 
	  //-run  the query against the mysql query function 
	  $result2=mysql_query($sql); 
	  //-create  while loop and loop through result set 
	  while($row=mysql_fetch_array($result2)){ 
	           
	          $ID=$row['pk_Payment_id']; 
	...
	//	echo  '<td><b>'  .$ID .'</b></td>'; 
		  echo '<tr>'; 
                    echo '<td><b>'. $row["pk_Payment_id"] .'</b></td>';
                     echo '<td><b>'. $row["fk_email_id"] .'</b></td>';
                    echo '<td><b>'. $row["fk_book_id"] .'</b></td>';
                    echo '<td><b>'. $row["Ticket_charge"] .'</b></td>'; 
					echo '<td><b>'. $row["Other_charge"] .'</b></td>';
